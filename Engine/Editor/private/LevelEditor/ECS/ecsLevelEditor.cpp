...
namespace GameEngine::EntitySystem::LevelEditorECS
{
	void RegisterLevelEditorEcsSystems(flecs::world& world, World::Level& level)
	{
		world.system<const PositionDesc, EntitySystem::EditorECS::Position>()
			.each([&](const PositionDesc& posDesc, EntitySystem::EditorECS::Position& position)
				{
					std::string descr = level.GetLevelObject(posDesc.objectId)->GetComponent(posDesc.componentId)->second;
					ParsePosition(descr, position);
				});
	}
}