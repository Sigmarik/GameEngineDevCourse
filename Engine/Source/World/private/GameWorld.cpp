...
namespace GameEngine::World
{
	...
	{
		for (const auto& [objectId, levelObject] : m_CurrentLevel.value().GetLevelObjects())
		{
			flecs::entity newEntity = m_World.entity(levelObject.GetName().c_str());

			for (const auto& [compId, objComponent] : levelObject.GetComponents())
			{
	...
	{
		for (const auto& [objId, levelObject] : m_CurrentLevel.value().GetLevelObjects())
		{
